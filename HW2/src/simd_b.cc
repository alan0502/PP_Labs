    ...
    int iters, int width, int *row_out)
{
    ...
    for (; i + 1 < width; i += 2) {
        ...
        __m128d cx  = _mm_add_pd(_mm_mul_pd(idx, v_dx), v_left);
        __m128d cy  = v_y0;
        ...
        for (int k = 0; k < iters; ++k) {
            ...
            // 判斷是否超過邊界
            __m128d mask = _mm_cmple_pd(r2, v_four);
            int mask_bits = _mm_movemask_pd(mask);
            if (mask_bits == 0)
                break;

            __m128d xy     = _mm_mul_pd(x, y);
            __m128d x_next = _mm_add_pd(_mm_sub_pd(x2, y2), cx);
            __m128d y_next = _mm_add_pd(_mm_mul_pd(v_two, xy), cy);

            // select(mask, new, old)
            x   = _mm_or_pd(_mm_and_pd(mask, x_next), _mm_andnot_pd(mask, x));
            y   = _mm_or_pd(_mm_and_pd(mask, y_next), _mm_andnot_pd(mask, y));
            itv = _mm_add_pd(itv, _mm_and_pd(mask, v_one));
        }
