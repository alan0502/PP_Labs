...
void write_png(const char* filename, int iters, int width, int height, const int* buffer) {
    ...
    // Every row is independent, we can parallelize this loop
...
int main(int argc, char** argv) {
    ...
    // local_count: number of elements to send from each process, sender's perspective
    // recvcounts: number of elements to receive from each process, root's perspective
    // displs: displacement (offset) for each process's data in the recv buffer, root's perspective
    int local_count = width * local_height;
    int *recvcounts = NULL, *displs = NULL;

    // recvcounts and displs are similar as counting sorted to distribute rows to buffers
    int total_count = 0;
    if (rank == 0) {
    ...
    // Gather all local images to root process by MPI_Gatherv
    // Non-root processes pass NULL for recvbuf, recvcounts, displs, but pass local_image and local_count
    // Root process allocates buffer to receive all data
    int* gathered = NULL;
    if (rank == 0)
