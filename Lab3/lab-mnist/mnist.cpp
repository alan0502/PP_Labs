...
// 就是 D = A * B + C，是 linear layer 中的 linear transformation
// A: 輸入資料
// B: 權重矩陣
// C: 偏差矩陣 (bias)
// D: 輸出資料
...
/* TODO: Parallel the for loops */
// Linear layer 中的 activation function
 ...
 */
 ...
 /* TODO: Parallel the for loops */
// 找出每一筆資料中 (row)，最大值所在的 index
// 即每筆輸入對應的預測類別 (argmax)
void Argmax(float *A, int *D, int n, int m) {
    #pragma acc parallel loop present(A, D)
    for (int i = 0; i < n; i++) {
        float mx = A[i * m];
        int index = 0;
        #pragma acc loop seq
...
void my_nn(float *training_images_flat, int num_images,
    ...
    #pragma acc data copyin(training_images_flat[0:num_images*LAYER0], \
                        ...
                        create(layer1_output[0:num_images*LAYER1], \
                        layer2_output[0:num_images*LAYER2])
    {
